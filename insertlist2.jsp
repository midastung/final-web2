      ...
      else
        {
          ...
          boolean gogo=true;
            ...
            for(int i=0;i<idd.length;i++)
            {
              ...
              rs=con.createStatement().executeQuery(sql);
              rs.next();
              sql="INSERT INTO list_shopping (p_id,m_account,l_name, l_email, l_address, l_country, l_cellphone, l_memo, l_payment, l_idd,l_number,l_totalprice,l_boolean) VALUES ('"+idd[i]+"','"+acc+"','"+firstname+"','"+email+"','"+address+"','"+country+"','"+tel+"','"+memo+"','"+payment+"','"+rr+"','"+number[i]+"','"+String.valueOf(Integer.valueOf(number[i])*Integer.valueOf(rs.getString("p_price")))+"','0')"; 
